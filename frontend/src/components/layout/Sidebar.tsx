    ...
    <>
      <aside className="w-64 bg-surface border-r border-border flex flex-col hidden md:flex">
        ...
        <div className="p-4 border-t border-border">
          <div 
            className="flex items-center gap-3 mb-4 px-2 p-2 -mx-2 rounded-lg hover:bg-surfaceHover cursor-pointer transition-colors group"
            onClick={() => setIsProfileModalOpen(true)}
          >
            <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold overflow-hidden shadow-sm border border-border group-hover:border-primary transition-colors">
              <img src="/profile-avatar.png" alt="Profile" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-medium text-text truncate group-hover:text-primary transition-colors">{user?.name}</p>
